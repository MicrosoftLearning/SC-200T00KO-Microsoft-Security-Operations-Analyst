...
**참고:** 가상 머신에 스크립트를 붙여넣을 때는 길이가 제한될 수 있습니다. 명령에서 직접 복사할 수 없는 경우 전체 스크립트를 가상 머신에 붙여 넣으려면 3개 섹션으로 나눠서 붙여 넣으세요.  메모장에서 연 *c2.ps1* 파일 내에서 스크립트가 여기에 나와 있는 지침과 동일하게 표시되는지 확인합니다.
...
메모장의 메뉴에서 **파일**, **저장**을 차례로 선택합니다. 명령 프롬프트 창의 각 행에 다음 명령을 입력하고 각 행의 끝에서 Enter 키를 누릅니다.
...
**참고:** 오류를 해결하라는 메시지가 표시됩니다. 정상적인 현상이므로 무시하세요.
**중요:** 아직 창을 닫지 마세요. 이 명령/PowerShell 스크립트를 백그라운드에서 실행하고 명령이 몇 시간 동안 로그 항목을 생성해야 합니다. 이 스크립트가 실행되는 동안 다음 작업과 다음 연습을 진행해도 됩니다. 이 작업에서 생성되는 데이터를 나중에 위협 헌팅 랩에서 사용합니다. 이 프로세스에서 대량의 데이터가 작성되거나 처리되지는 않습니다.
