...
여러분은 Microsoft Defender for Cloud를 사용하여 클라우드 워크로드 보호를 구현하고 있는 회사에서 일하는 보안 작업 분석가입니다.  이 랩에서는 Microsoft Defender for Cloud를 사용하도록 설정합니다.
...
7. *"Azure Pass - 스폰서쉽*구독(또는 선택한 언어에서 해당하는 이름)이 표시되면 작업 2로 진행합니다. 표시되지 않을 경우 강사에게 사용자의 테넌트 관리자 사용자 자격 증명으로 Azure 구독을 만드는 방법을 물어봅니다.
...
이 작업에서는 Microsoft Defender for Cloud에서 사용할 Log Analytics 작업 영역을 만듭니다.
...
5. 이름으로는 *uniquenameDefender*와 같은 고유한 이름을 입력합니다.

6. **검토 + 만들기**를 선택합니다.

7. 작업 영역 유효성 검사에 통과하면 **만들기**를 선택합니다. 새 작업 영역이 프로비전될 때까지 기다립니다. 몇 분 정도 걸릴 수 있습니다.

### 작업 3: Microsoft Defender for Cloud를 사용하도록 설정

이 작에서는 Microsoft Defender for Cloud를 사용하도록 설정하고 구성합니다.

1. Azure Portal의 검색 창에 *Defender*를 입력하고 **Microsoft Defender for Cloud**를 선택합니다.

2. **시작** 페이지에서 **업그레이드** 섹션으로 이동하여 구독이 선택되어 있는지 확인하고 페이지 아래쪽의 **업그레이드** 단추를 선택합니다.

3. 구독에 이미 포함되어 있는 가상 머신에 에이전트를 설치하는 옵션이 다음 페이지에 표시됩니다. 여기서는 아무 작업도 수행하지 않습니다.

4. 포털 메뉴의 관리 영역에서 **환경 설정**을 선택합니다.

5. "Defender 적용 범위"에 11개 이상의 플랜이 포함?있음을 확인합니다. 표시되지 않는 경우 *평가판이 시작되었습니다* 알림이 표시될 때까지 기다립니다. 알림이 표시되면 **새로 고침**을 선택하고 구독을 선택합니다. 

6. *Microsoft Defender for* 및 *가격*에서 다양한 Azure 리소스와 요금을 검토합니다.

7. 설정 영역에서 **자동 프로비전**을 선택합니다.

8. 자동 프로비전 - 확장을 검토합니다. **Azure VM용 Log Analytics 에이전트**가 **꺼짐**으로 설정되어 있는지 확인합니다.

9. 페이지 오른쪽 위에 있는 'x'를 선택하여 설정 페이지를 닫고 **환경 설정** 페이지로 다시 돌아가 구독 왼쪽에 있는 '>'를 선택합니다.

10. 앞에서 만든 Log Analytics 작업 영역 *uniquenameDefender*를 선택하여 사용 가능한 옵션 및 가격을 검토합니다.
...
11. *서버 세부 정보* 및 *네트워크 연결* 옵션을 검토합니다. **다음**을 선택하여 태그 탭으로 이동합니다.

12. **다음**을 선택하여 스크립트 다운로드 및 실행 탭으로 이동합니다.

13. **등록**을 선택합니다.

**참고:** 처리가 완료될 때까지 3분 정도 기다립니다.
...
23. PowerShell 출력 마지막 줄에 있는 지침을 따라 디바이스 등록을 완료합니다.  여기에는 브라우저를 통한 디바이스 인증이 포함됩니다.  URL(https://microsoft.com/devicelogin)을 복사하여 새 Edge 브라우저 탭에 입력합니다. Windows PowerShell 창으로 돌아가 인증을 위해 코드를 복사하고 이전에 열어둔 탭에 붙여 넣은 후 **다음**을 선택합니다. 테넌트 관리자 계정을 선택하고 *Azure Connected Machine 에이전트에 로그인하시겠습니까?* 창에서 **계속**을 선택합니다. 

24. Windows PowerShell 창에 *"리소스를 Azure에 성공적으로 온보딩했습니다.* 메시지가 표시되면 스크립트를 다운로드한 Azure Portal 페이지로 이동하여 **닫기**를 선택합니다. **Azure Arc용 서버 추가**를 닫고 Azure Arc **서버** 페이지로 돌아갑니다.
...
1. "Microsoft Defender for Cloud"로 이동하여 **시작** 페이지를 선택합니다.

2. **시작** 탭을 선택합니다.

3. 아래로 스크롤하여 *비 Azure 서버 추가* 섹션에서 **구성**을 선택합니다.

4. 앞에서 만든 작업 영역 옆의 **업그레이드**를 선택합니다.  이 작업은 몇 분 정도 걸릴 수 있습니다. *"작업 영역용 Defender 플랜을 저장했습니다."* 알림이 표시될 때까지 기다립니다.
...
9. Azure Portal에 표시된 **작업 영역 ID**와 **기본 키**를 복사하여 마법사 페이지의 각 필드에 붙여 넣은 후에 **다음**을 선택합니다.

10. 설치를 계속 진행합니다. 완료되면 **마침**을 선택합니다.

11. "Microsoft Defender for Cloud" 포털로 이동하여 **인벤토리**를 선택합니다.

12. 목록에 서버가 표시됩니다.  업데이트를 확인하려면 **새로 고침**을 선택해야 할 수 있습니다. 업데이트가 표시될 때까지 몇 분 정도 걸릴 수 있습니다.

13. 다음 랩으로 이동하고 나중에 **Microsoft Defender for Cloud**로 돌아올 수 있습니다. 서버가 **인벤토리** 섹션에 표시됩니다. 
